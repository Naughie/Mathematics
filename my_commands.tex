...
\def\defineTitle(#1;#2;#3){\csname #1\endcsname#3{\ROMAN{#2}}}
\def\myTitle{(title;Notes of Mathematics;)}
\def\myAuthor{(author;Masato Nakata;[*])}
\def\myAffil{(affil;Department of Science, Kyoto University;[*])}
\def\myDate{(date;Since Aug 27, 2017;)}
\@for\next:=\myTitle,\myAuthor,\myAffil,\myDate\do{%
  \expandafter\expandafter\expandafter\defineTitle\next}
