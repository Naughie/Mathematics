...
\subsec{Manifolds on Euclidean Spaces}
...
A $k$-linear function $f \colon V^k \to \bb R$ on a vector space $V$ is called a \EMPH{k-tensor} on $V$. The vector space of all $k$-tensors on $V$ is denoted by $L_k(V)$. $k$ is called the degree of $f$.
