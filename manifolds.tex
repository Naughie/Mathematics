...
A \empha{vector field} on $U \in \mathcal{O}_n$ is a map $X \colon U \to T_p(\bb{R}^n)$. $X = \sum a^i \partial / \partial x^i$ means
\[
...
A \empha{covector field} or a \empha{differential 1-form} on $U \in \mathcal{O}_n$ is $\omega \colon U \to \bigcup_{p \in U} T^*_p (\bb{R}^n)$ that maps $U \ni p \mapsto \omega_p \in T^*_p (\bb{R}^n)$.
...
For $f \in C^\infty (U)$ on $U \in \mathcal{O}_n$, the \empha{differential} $df$ of $f$ is a differential 1-form defined by
\[
(df)_p (X_p) = X_p f.
\]
