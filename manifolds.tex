...
The \empha{tangent space} $T_p (\bb{R}^n)$ at $p \in \bb{R}^n$ is the set of arrows from $p$.

\subsubsection{Directional derivative [Definition \ref{directional-derivative}]}\label{directional-derivative}
The \empha{directional derivative} of a smooth function $f$ in the direction $v \in T_p(\bb{R}^n)$ at $p \in \bb{R}^n$ is
\[
...
A linear map $D \colon C_p^\infty \to \bb{R}$ satisfying the Leibniz rule (i.e., $D(fg) = (D f)g(p) + f(p)D g$ for any $f, g \in C_p^\infty$) is called a \empha{derivation} at $p$ or a \empha{point-derivation} of $C_p^\infty$.
...
A \empha{vector field} on $U \in \usualtop$ is a map $X \colon U \to T_p(\bb{R}^n)$. $X = \sum a^i \partial / \partial x^i$ means
\[
...
A $k$-linear function on a vector space $V$ $f \colon V^k \to \bb{R}$ is called a \empha{k-tensor} on $V$. The vector space of all $k$-tensors on $V$ is denoted by $L_k(V)$. $k$ is called the degree of $f$.
...
A $k$-tensor $f \colon V^k \to \bb{R}$ is \empha{symmetric} if
\[
\forall \sigma \in \g S_k,\ \sigma f  =f,
\]
and $f$ is \empha{alternating} if
\[
...
An alternating $k$-tensor on a vector space $V$ is also called a \empha{k-covector} or a \empha{multicovector of degree k} on $V$. The set of all $k$-covectors on $V$ is denoted by $A_k(v)$ for $k > 0$; for $k = 0$, $A_0 (V) = \bb{R}$.
...
For $f \in L_k(V), g \in L_\ell(V)$ on a vector space $V$, the \empha{tensor product} $f \otimes g \in L_{k+\ell}(V)$ is defined by
\[
...
For $f \in A_k(V), g \in A_\ell(V)$ on a vector space $V$, their \empha{wedge product} or \empha{exterior product} is
\begin{align*}
...
An algebra $\bb{A}$ over a field $\bb{K}$ is said to be \empha{graded} if $\bb{A} = \bigoplus_{k = 0}^\infty A^k$ is a direct sum of vector spaces over $\bb{K}$ such that the multiplication sends $A^k \times A^l$ to $A^{k+l}$. $A = \bigoplus_{k = 0}^\infty A^k$ means each nonzero $a \in \bb{A}$ is uniquely a finite sum $a = a_{i_1} + \dotsb a_{i_m}$ where nonzero $a_{i_j} \in A^{i_j}$.

$\bb{A}$ is \empha{anticommutative} or \empha{graded commutative} if $\forall a \in A^k, b \in A^\ell$, $ab = (-1)^{k\ell} ba$.

A \empha{homomorphism} of graded algebras is an algebra homomorphism that preserves the degree.
...
For a vector space $V$ of degree $n < \infty$, the \empha{exterior algebra} or the \empha{Grassmann algebra} of multicovectors on $V$ is the anticommutative graded algebra
\[
...
The \empha{cotangent space} to $\bb{R}^n$ at $p$ is $T^*_p (\bb{R}^n) = (T_p(\bb{R}^n))^*$.
...
A \empha{covector field} or a \empha{differential 1-form} on $U \in \usualtop$ is $\omega \colon U \to \bigcup_{p \in U} T^*_p (\bb{R}^n)$ that maps $U \ni p \mapsto \omega_p \in T^*_p (\bb{R}^n)$.
...
For $f \in C^\infty (U)$ on $U \in \usualtop$, the \empha{differential} $df$ of $f$ is a differential 1-form defined by
\[
(df)_p (X_p) = X_p f.
\]
...
A differential 1-form $\omega = \sum a_i dx^i$ on $U \in \usualtop$ is \empha{smooth} if all $a_i \colon U \to \bb{R}$ are smooth.
...
A \empha{differential k-form} or \empha{differential form of degree k} on $U \in \usualtop$ is $\omega \colon U \ni p \mapsto \omega_p \in A_k (T_p(\bb{R}^n))$.
...
\]
$\omega$ is \empha{smooth} if all $a_I \colon U \to \bb{R}$ are smooth. The vector space of $C^\infty$  differential $k$-forms on $U$ is denoted by $\Omega^k(U)$. If $k = 0$, $\Omega^0(U) = C^\infty(U)$.
...
For differential $k$-form $\omega$ and $\ell$-form $\tau$ on $U \in \usualtop$, their \empha{wedge product} $\omega \wedge \tau$ is a differential $(k+\ell)$-form defined by
\[
